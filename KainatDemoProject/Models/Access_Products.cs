using System.Configuration;
using System.Data;
using System.Data.SqlClient;
using KainatDemoProject.Models;
using System.Linq;
using System.Web;
using System;
using System.Collections.Generic;
public class Access_Products

{

    SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["con"].ConnectionString);

    //Display all record
    public DataSet get_record()

    {

        SqlCommand com = new SqlCommand("Sp_register_get", con);

        com.CommandType = CommandType.StoredProcedure;

        SqlDataAdapter da = new SqlDataAdapter(com);

        DataSet ds = new DataSet();

        da.Fill(ds);

        return ds;

    }

    // Update record

    public void update_record(register rs)

    {

        SqlCommand com = new SqlCommand("Sp_register_Update", con);

        com.CommandType = CommandType.StoredProcedure;

     //   com.Parameters.AddWithValue("@Sr_no", rs.Sr_no);

        com.Parameters.AddWithValue("@pn", rs.pn);

        com.Parameters.AddWithValue("@pcat", rs.pcat);

        com.Parameters.AddWithValue("@pprice", rs.pprice);

        com.Parameters.AddWithValue("@pavail", rs.pavail);

        con.Open();

        com.ExecuteNonQuery();

        con.Close();

    }

    // Get Record by id

    public DataSet get_recordbyid(int id)

    {

        SqlCommand com = new SqlCommand("Sp_register_byid", con);

        com.CommandType = CommandType.StoredProcedure;

        com.Parameters.AddWithValue("@Sr_no", id);

        SqlDataAdapter da = new SqlDataAdapter(com);

        DataSet ds = new DataSet();

        da.Fill(ds);

        return ds;

    }

    // Delete record

    public void deletedata(int id)

    {

        SqlCommand com = new SqlCommand("Sp_register_delete", con);

        com.CommandType = CommandType.StoredProcedure;

        com.Parameters.AddWithValue("@Sr_no", id);

        con.Open();

        com.ExecuteNonQuery();

        con.Close();

    }

    public void Add_record(register rs)

    {

        SqlCommand com = new SqlCommand("Sp_register", con);

        com.CommandType = CommandType.StoredProcedure;

        //com.Parameters.AddWithValue("@Sr_no", rs.Sr_no);

        com.Parameters.AddWithValue("@pn", rs.pn);

        com.Parameters.AddWithValue("@pcat", rs.pcat);

        com.Parameters.AddWithValue("@pprice", rs.pprice);

        com.Parameters.AddWithValue("@pavail", rs.pavail);

        con.Open();

        com.ExecuteNonQuery();

        con.Close();

    }

}